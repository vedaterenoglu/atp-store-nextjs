...
describe('Products Error Page', () => {
  ...
  describe('Reset functionality', () => {
    ...
    it('should handle multiple reset clicks', () => {
      const testError = new Error('Test error')
      const localMockReset = jest.fn()

      render(<ErrorPage error={testError} reset={localMockReset} />)

      const button = screen.getByRole('button', { name: 'Try again' })

      // The button should allow multiple clicks
      fireEvent.click(button)
      fireEvent.click(button)
      fireEvent.click(button)

      // In reality, the error boundary might only call reset once per error
      // but the button itself should allow multiple clicks
      expect(localMockReset).toHaveBeenCalled()
      expect(localMockReset.mock.calls.length).toBeGreaterThanOrEqual(1)
    })
  })
