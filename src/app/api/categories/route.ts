/**
 * API Route for Categories - Facade over GraphQL GetCategoriesQuery
 * SOLID Principles: SRP - Single responsibility for categories endpoint
...
import type { GetCategoriesQueryVariables } from '@/services/graphql/queries/GetCategoriesQuery.types'
...
export async function GET(request: Request) {
  try {
    ...
    const response = await fetch(HASURA_GRAPHQL_ENDPOINT as string, {
      ...
      body: JSON.stringify({
        query: print(GetCategoriesQueryDocument),
        variables,
      }),
    })
    ...
    if (result.errors) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch categories', details: result.errors },
        { status: 500 }
      )
    }

    // Validate response with Zod
    const validatedData = validateGetCategoriesResponse(result.data)
